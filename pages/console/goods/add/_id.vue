        ...
        <div class="goods-add__form">
            <md-field title="填写商品信息">
                ...
                <md-field-item
                    ...
                    arrow="arrow-right"
                    align="right"
                    :content="addForm.usernameExpire"
                    @click.native="isDatePickerShow = true">
                </md-field-item>
                <md-field-item title="同一时间出租次数">
                    <md-stepper slot="right" v-model="addForm.stockTotal" :step="1" :min="1" :max="5" is-integer />
                </md-field-item>
                ...
                <div class="add-btn-box">
                    <md-button @click="addSubmit" :loading="subLoading" :inactive="subLoading" type="primary">确认发布</md-button>
                </div>
                
            </md-field>
        </div>
        <div class="add-goods-protocol">
            <p class="sub-tips">发布商品说明：共享经济 请合理合法共享！</p>
            <p>1. 请保证自己发布的商品信息真实可用性，合理（共享、出租、合租、转让）自己的商品信息。</p>
            <p>2. 平台保证加密保存你的商品信息，绝不会向非买家泄露，请放心发布你的商品信息。</p>
            <p>3. 号主需自行承担（共享、出租、合租、转让）商品被冻结、封停等影响商品正常使用的风险。</p>
            <p>4. 如果已发布的商品信息（密码）发生改变，请及时更新商品信息，交易中的用户可看到最新信息（密码）。</p>
            <p>5. 商品（共享、出租、合租、转让）的收益，一旦交易产生，平台收到买家按商品要求支付指定金额，该笔资金会在号主钱包进行冻结直至订单结束（出租时间到期），号主即可提现至支付宝、微信、银行卡。</p>
            <p>6. 平台只提供商品信息托管服务，不能保证商品交易过程中，被买家非法利用（例如：申诉盗号行为），请再次确保商品已绑定自己手机号，随时能找回商品的密码。</p>
            <p>7. 每一个订单交易，平台将收取 5% 交易手续费（订单金额低于1元，只收取0.01元），平台自动从订单收益中扣除。</p>
        </div>
        <!-- 商品分类选择 -->
        <md-selector
...
export default {
    ...
    head () {
        return {
            title: '发布商品 - 店铺工作台',
            meta: [
                { hid: 'description', name: 'description', content: '号子短租，发布商品' }
            ]
        }
    },
    fetch({store, route, params}) {
        store.commit('SET_HEAD_TITLE', '发布商品');
    ...
    computed: {
        ...
        getUploadedImgNum() {
            return this.addForm.descriptionImages && this.addForm.descriptionImages.length || 0
        }
    },
    created() {
    ...
    mounted() {
        ...
        this.addForm.shelfId = id;
        this.minDate = new Date();
        this.currentDate = new Date();
        this.$set(this.addForm, 'usernameExpire', this.$Utils.formatDate(this.currentDate));
    ...
    methods: {
        ...
        verifyForm () {
            ...
            let { categoryId, goodsName, username, password, usernameExpire, stockTotal, description } = this.addForm;
            if(!categoryId) {
            ...
            //保存规格信息
            this.addForm.sku = this.timeSpecList;
            if(!description) {
        ...
        async addSubmit() {
            if(this.verifyForm()) {
                ...
                if(result === 'ok') {
                    //价格 * 100
                    let sku = this.addForm.sku.map(item => {
                        return {
                            ...item,
                    ...
                    //等待图片上传完毕之后，开始发布
                    let { code, msg } = await this.$api.releaseBenchGoods(params, this.getAppToken);
        ...
        startUploadImg() {
            return new Promise(async (resolve, reject) => {
                ...
                try {
                    ...
                    this.uploadedNum = 0;
                    this.addForm.descriptionImages = [];
                    for(let i=0; i<list.length; i++) {
                        ...
                        if(code == 200) {
                            ...
                            this.addForm.descriptionImages.push(IMAGE_URL + data.url);
                            this.uploadedNum = this.addForm.descriptionImages.length;
                        }
                    }
                    console.log(this.addForm);
                } catch (error) {
        ...
        onDatePickerConfirm(columnsValue) {
            ...
            this.addForm.usernameExpire = this.$refs.datePicker.getFormatDate('yyyy-MM-dd');
            console.log(this.addForm.usernameExpire);
        },
        //data = {dataUrl, blob}
